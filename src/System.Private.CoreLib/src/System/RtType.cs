    ...
    internal sealed class RuntimeType : TypeInfo, ICloneable
    {
        ...
        internal static void ValidateGenericArguments(MemberInfo definition, RuntimeType[] genericArguments, Exception? e)
        {
            ...
            else
            {
                ...
                RuntimeType? declaringType = genericMethodDefinition.DeclaringType as RuntimeType;
            ...
            Type[]? types, ParameterModifier[]? modifiers)
        {
            ...
            {
                MethodInfo firstCandidate = candidates[0]!; // TODO-NULLABLE: https://github.com/dotnet/roslyn/issues/34644
                ...
                else if (types == null)
                {
                    ...
                    {
                        MethodInfo methodInfo = candidates[j]!; // TODO-NULLABLE: https://github.com/dotnet/roslyn/issues/34644
            ...
            string name, BindingFlags bindingAttr, Binder? binder, Type? returnType, Type[]? types, ParameterModifier[]? modifiers)
        {
            ...
            if (types == null || types.Length == 0)
            {
                ...
                if (candidates.Count == 1)
                {
                    ...
                    if (returnType != null && !returnType.IsEquivalentTo(firstCandidate.PropertyType))
                        return null;
            ...
            object[]? providedArgs, ParameterModifier[]? modifiers, CultureInfo? culture, string[]? namedParams)
        {
            ...
            if (IsGetField || IsSetField)
            {
                ...
                if (GetMember(name, MemberTypes.Field, bindingFlags) is FieldInfo[] flds)
                {
                    if (flds.Length == 1)
                    {
                        selFld = flds[0];
            ...
            if ((bindingFlags & BindingFlags.InvokeMethod) != 0)
            {
                ...
                // Lookup Methods
                if (GetMember(name, MemberTypes.Method, bindingFlags) is MethodInfo[] semiFinalists)
                {
